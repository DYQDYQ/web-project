  		...
  		if(data.id != "#" && data.id != "model") {
  			...
  			if(nodetype == "unitprj") {
  				...
  				bChangeModel = true;
					...
					getrelatefilelist("单位工程",data.node.id.split('_')[1]);
  				...
  				getrelatefilelist("楼层",data.node.id.split('_')[1]);
  			} else if(nodetype.indexOf("zone") != -1) {
  				_selZones = _selZones + data.node.id.split('_')[1] + ",";
  				getrelatefilelist("分区",data.node.id.split('_')[1]);
  			} else {
  ...
  function initPbTypeTree() {
  	...
  	$("#jstree_div_pbtype").on("select_node.jstree", function(evt, data) {
  		...
  		if(data.id != "#" && data.id != "pbtypetree") {
  			...
  			var nodetype = data.node.id.split('_')[0];
  			if(nodetype == "major") {
  				getrelatefilelist("专业",data.node.id.split('_')[1]);
  				...
  				getrelatefilelist("构件类型",data.node.id.split('_')[1]);
  ...
  function getPbStatusByMajor() {
  	$.ajax({
  		...
  		success: function(data) {
  			...
  			$("#pbstatussel").empty();
				$("#pbstatussel").append("<option value=''>全部</option>");
...
function filterPbByCountType(counttype,obj){
			
			if($( obj ).hasClass("label-default")){
							$("#labelCountType a" ).removeClass("label-danger").addClass("label-default");
	  	...
	  	$.ajax({
  		type: "get",
  		url: "/task/modelview/filterPbByCountType/",
  		cache: false,
  		dataType: "json",
  		data: {
  			"counttype": counttype,
