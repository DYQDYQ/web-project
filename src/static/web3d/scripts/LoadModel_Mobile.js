...
function loadModelFilelist(){
	      $.ajax({
				  type:"get",
				  url: " /task/modelfiles/?perPage=999999",
				  cache:false,
				  dataType:"json",
				  success: function(data){
				  	var html = '';
				 	for(var each in data.results){
				 		if(_curUnitId == data.results[each].relatedunitprojectid && _curMajor == data.results[each].relatedmajorid){
				 			html+='<option value ="'+data.results[each].id+'" selected>'+data.results[each].unitprojectname+"-"+data.results[each].majorname+'</option>'
				 		}else{
				 			html+='<option value ="'+data.results[each].id+'">'+data.results[each].unitprojectname+"-"+data.results[each].majorname+'</option>'
				 		}
